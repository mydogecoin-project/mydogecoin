...
const char * const BITCOIN_CONF_FILENAME = "mydogecoin.conf";
const char * const BITCOIN_PID_FILENAME = "mydogecoind.pid";
...
#else
    const char* pszModule = "mydogecoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "Mydogecoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/Mydogecoin";
#else
    // Unix
    return pathRet / ".mydogecoin";
#endif
#endif
}
