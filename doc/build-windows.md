...
Below are some notes on how to build Mydogecoin Core for Windows.
...
To build Mydogecoin from source code, you'll need the source code. Either check it out via `git` or download
a zip file. (Look at the green "<> Code" button on [the Mydogecoin GitHub repository](https://github.com/mydogecoin/mydogecoin/)).
...
Note that for WSL v1 the Mydogecoin Core source path MUST be somewhere in the default mount file system, for
example /usr/src/mydogecoin, AND not under, for example, /mnt/d/mydogecoin.
...
Ensure that this toolchain can build for `posix` (else you may have compilation errors for Mydogecoin's dependencies):
...
way. This will install to `c:\workspace\mydogecoin`, for example:

    make install DESTDIR=/mnt/c/workspace/mydogecoin
