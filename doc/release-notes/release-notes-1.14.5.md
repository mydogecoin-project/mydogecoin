Mydogecoin Core version 1.14.5 is now available from:

  <https://github.com/mydogecoin/mydogecoin/releases/tag/v1.14.5/>

This is a new minor version release, including important security updates and
changes to network policies. All Mydogecoin Core users, miners, services, relay
...
most Mydogecoin Core users.

### Remote Code Execution in Mydogecoin QT (CVE-2021-3401)
...
**Mydogecoin QT users are urged to please update their installations to this
...
leakage of sensitive information when Mydogecoin Core crashes on Unix platforms.
The vulnerability is patched for systems that run a Linux kernel equal to or
higher than 3.4.

**Mydogecoin Core wallet users on Linux platforms are urged to please update to
this version.**
...
the Mydogecoin network, following the reduction of relay and mining defaults in
...
* the recommended minimum transaction fee is 0.01 MYDOGE/kb, and
* the recommended dust limit is 1 MYDOGE, and
* the recommended RBF increment is 0.001 MYDOGE.

### Wallet/UI Changes

* The user interface for selecting fees when transacting MYDOGE has been updated
  to give an idea of how much is being spent, rather than a block target. As
  Mydogecoin blocks are not full, typically all transactions are mined in the next
  block, and therefore the target estimation does not makes sense for Mydogecoin.
...
* The default minimum transaction fee is now 0.01 MYDOGE per kilobyte. Note that
...
* Introduce `-discardthreshold`, a wallet-specific, configurable dust limit that
  ...
  default dust limit is recommended to stay at 1 MYDOGE, as versions 1.14.2 until
  1.14.4 have a bug that rejects any transaction with an output under 1 MYDOGE.
...
* Split the dust limit into a hard and soft threshold, to reintroduce the
  ...
  * `-harddustlimit` is by default set at 0.001 MYDOGE and sets the value under
...
The Berkley DB version used by Mydogecoin Core has been updated to 5.3 (from 5.1)
...
performance of cryptographic routines within Mydogecoin Core.
