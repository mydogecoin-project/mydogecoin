# Mydogecoin Core 1.8.1

Mydogecoin Core 1.8.1 is primarily a bugfix release, bringing Mydogecoin Core in
line with Bitcoin 0.9.3. Mydogecoin Core 1.8.1 also adds in support for printing
...
* Disable Transifex based translation process (not used by Mydogecoin Core)
* Add checkpoints around AuxPoW switchover block (371,337)
* Disable connections from pre-1.8 Mydogecoin Core clients
...
There is a known issue with quantities in JSON used by the RPC interface (https://github.com/mydogecoin/mydogecoin/issues/134).
...
This only affects values significantly below 1 Mydoge, however merchants may wish to round to
the nearest Mydoge on all transactions for now.
